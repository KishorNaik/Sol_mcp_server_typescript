...
//#region Domain Event
export class TodoCreateDomainEvent extends NotificationData {
...
@sealed
@notificationHandler(TodoCreateDomainEvent)
export class TodoCreateDomainEventHandler implements NotificationHandler<TodoCreateDomainEvent> {
	private readonly _redisCacheService: ITodoSharedCacheService;
	public constructor() {
		this._redisCacheService = Container.get(TodosSharedCacheService);
	}

	public async handle(notification: TodoCreateDomainEvent): Promise<void> {
		try {
			// Set Cache
